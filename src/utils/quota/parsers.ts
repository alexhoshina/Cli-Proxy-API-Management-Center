...
import type { ClaudeUsagePayload, CodexUsagePayload, CopilotUsagePayload, GeminiCliQuotaPayload, KimiUsagePayload } from '@/types';
...
export function parseCopilotUsagePayload(payload: unknown): CopilotUsagePayload | null {
  if (payload === undefined || payload === null) return null;
  if (typeof payload === 'string') {
    const trimmed = payload.trim();
    if (!trimmed) return null;
    try {
      return JSON.parse(trimmed) as CopilotUsagePayload;
    } catch {
      return null;
    }
  }
  if (typeof payload === 'object') {
    return payload as CopilotUsagePayload;
  }
  return null;
}
